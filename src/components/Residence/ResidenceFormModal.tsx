...
interface ResidenceFormModalProps {
  isOpen: boolean;
  mode: 'create' | 'edit';
  isLoading: boolean;
  isSaving: boolean;
  ...
  setFormData: React.Dispatch<React.SetStateAction<ResidenceFormData>>;
}

const ResidenceFormModal: React.FC<ResidenceFormModalProps> = ({
  isOpen,
  mode,
  isLoading,
  isSaving,
      ...
      <div className="bg-white rounded-lg shadow-xl w-full max-w-lg overflow-hidden flex flex-col">
        ...
        <div className="px-6 py-4 border-b border-slate-200 flex justify-between items-center bg-slate-50">
          <h3 className="text-lg font-bold text-slate-800">
            {mode === 'edit' ? 'Edit Data Rumah' : 'Tambah Rumah Baru'}
          </h3>
          <button 
            onClick={onClose}
        ...
        <div className="p-6 overflow-y-auto max-h-[70vh]">
          {isLoading ? (
            <div className="flex flex-col items-center justify-center py-12 text-slate-500">
              ...
              <span>Memuat data rumah...</span>
            </div>
          ) : (
            <form id="residenceForm" onSubmit={onSubmit} className="space-y-4">
              {error && (
              ...
              <div className="space-y-1">
                <label className="text-sm font-semibold text-slate-700">Nomor Rumah</label>
                <input 
                  type="text"
              ...
              <div className="space-y-1">
                ...
                <textarea 
                  ...
                  rows={3}
                  className="w-full px-3 py-2 border border-slate-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:outline-none transition-all resize-none"
                  required
                />
              </div>
            </form>
...
export default ResidenceFormModal;
