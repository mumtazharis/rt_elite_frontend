...
import { PlusCircle, Settings, Calendar } from 'lucide-react';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      Swal.fire({
        icon: 'success',
        title: 'Berhasil!',
        text: isEdit ? 'Pengeluaran berhasil diperbarui' : 'Pengeluaran manual berhasil dicatat',
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-800">Daftar Pengeluaran</h1>
          <p className="text-sm text-slate-500">Pantau seluruh pengeluaran kas RT.</p>
        </div>
        <div className="flex items-center gap-2">
            <button 
            onClick={() => navigate('/keuangan/pengeluaran/setting')}
            className="bg-slate-100 text-slate-700 px-4 py-2 rounded-md font-semibold hover:bg-slate-200 transition-colors flex items-center gap-2 shadow-sm border border-slate-200"
            >
            <Settings size={20} />
            Pengaturan Rutin
            </button>
            ...
            className="bg-red-600 text-white px-4 py-2 rounded-md font-semibold hover:bg-red-700 transition-colors flex items-center gap-2 shadow-sm"
            >
            <PlusCircle size={20} />
            Tambah Pengeluaran
            </button>
